...
function format(option) {
	...
	var ob = '<span class="country-select"><span class="flag ' + jQuery(option.element).data('image') + '"></span></span><span>'+option.text+'</span>';
	return ob;
};
...
function getSmsPricing(){	
	...
	// }
	// data['apiUrl'] =  (data['country'] === 'India') ? 'https://control.msg91.com/action_layer.php' : 'https://world.msg91.com/action_layer.php';
...
function makeApiCall(data, type) {
	...
	jQuery.ajax({
		...
		success: function(result){
			...
			if (type == 'EMAIL_NEW') {//called from Pricing Email tab & from email page
				__EMAIL_PRICING_DATA = obj.data;
...
function drawNewEmailPricingUI() {
	...
	//jQuery('#loadingmessage').show(); comment by kuldeep
	if (__EMAIL_PRICING_DATA && __EMAIL_PRICING_DATA.length > 0) {
		...
		__EMAIL_PRICING_DATA.forEach(item => {
			if (item.is_active) {
				...
				html += '<div class="mainblueemail"><h3 class="bluetopcircle">'+item.name+'</h3></div>';

				let obj = item.plan_amounts.find(o => o.currency_id == selectedCurrency && o.plan_type.name === selectedMode);
				if (obj) {
					let symbol;
					...
					html += '<h4 class="monthsystem"><span id="amountPerSmsEmail"></span> <span class="currencytext_email"></span> ' +cstr+'</h4>';
					item.plan_services.forEach(o => {
						html += '<p class="amountnewemail">'+o.service_credit.free_credits+' '+o.service_credit.service.name+'</p><a class="sms-free-bt getfreenewbuton" href="https://control.msg91.com/signup/" target="_blank">Get Started</a>';
					});
					html += '<div class="mainaddon"><h5 class="addsonemail">Add-Ons</h5>';
					item.plan_services.forEach(o => {
						let __rtObj = o.service_credit.service_credit_rates.find(o => o.currency_id == selectedCurrency);
						if (__rtObj) {
							html += '<h6 class="peremailprice">'+symbol+__rtObj.follow_up_rate+' per '+o.service_credit.service.name+'</h6>';
						}
					});
					html += '</div>';
				}
		...
		//jQuery('#loadingmessage').hide(); comment by kuldeep
	}
}
...
function getEmailPricingNew() {
	var data= {};
	//data['apiUrl'] = 'http://subscriptions.taskb.in/api/plans?ms_id=1';//old way changed on 01Feb2022
	data['apiUrl'] = 'https://subscriptions.elitevysya.com/api/plans?ms_id=1';
	...
	$(".country3").select2({
		...
		templateSelection: function(option) {
			if (option.id.length > 0) {
				return '<span class="country-select"><span class="flag ' + jQuery(option.element).data('image')+'"></span></span><span>'+option.text+'</span>';
			} else {
				return option.text;
			}
		},
